...
```{r ,warning=F,message=F}
RidgePlot(data.filt, features = c("Mcm5"))
RidgePlot(data.filt, features = c("Pcna"))
RidgePlot(data.filt, features = c("Cdk1"))
# Mcm5 initiation of DNA replication (G1/S)
# Pcna leading strand synthesis during DNA replication (S)
# Cdk1 M-phase promoting factor (G2M)
```

```{r ,warning=F,message=F}

data.filt <- RunPCA(data.filt, verbose=F)
DimPlot(data.filt, reduction="pca", group.by = "Phase")
...
Also, we clearly see an un-even distribution of G2M and S data that could be strictly associated to their phase state.

## Regress out cell cycle scores during data scaling

We try to eliminate ('regress out') this factor of diversity from the dataset. Seurat, for each gene, constructs a model to understand the connection between gene expression and the S and G2M cell cycle scores. The adjusted expression matrix, obtained by analyzing the scaled residuals of this model, serves as a 'corrected' dataset for subsequent dimensional reduction. Following this regression step, performing PCA on the variable genes no longer yields components linked to the cell cycle.
...
DimPlot(data.filt, reduction = "pca",label = F, group.by = "old.ident") +  theme(legend.position = "none")
```
...
data.filt <- RunTSNE(data.filt, 
                     dims = 1:50)
DimPlot(data.filt, reduction = "tsne", group.by = "old.ident") + theme(legend.position = "none")
```
...
DimPlot(data.filt, reduction = "umap", group.by = "old.ident") + theme(legend.position = "none")
```
...
markers <- FindAllMarkers(data.filt, 
                               only.pos = TRUE, # use only positive markers
                               min.pct = 0.25,  # only genes that are detected in minimum 25%
                               logfc.threshold = 0.25) # test only genes with minimum 0.25 fold-difference
...
FeaturePlot(data.filt, features = list_of_markers[5:8])
FeaturePlot(data.filt, features = list_of_markers[9:10])
```

```{r}
VlnPlot(data.filt, features = list_of_markers[0:5], pt.size = 0)
VlnPlot(data.filt, features = list_of_markers[6:10], pt.size = 0)
```
